...
setup.describe('Staging Environment Setup', () => {
  ...
  setup('validate core API endpoints', async ({ request }) => {
    ...
    const paymentMethodsResponse = await request.get(`${stagingConfig.API_BASE_URL}/v1/billing/payment-methods`, {
      headers: { Cookie: cookieHeader },
    });
  ...
  setup('validate webhook processing capability', async ({ request }) => {
    ...
    const webhookResponse = await request.post(`${stagingConfig.API_BASE_URL}/v1/billing/webhook/stripe`, {
      data: { test: 'ping' },
      headers: {
