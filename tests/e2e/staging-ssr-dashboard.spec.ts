...
test.describe('SSR Dashboard - Staging', () => {
  ...
  test('should validate cookie forwarding for SSR', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="session-chip"]')).toContainText(
      stagingConfig.TEST_USER.email
    );
  ...
  test('should validate data freshness and caching', async ({ page }) => {
    ...
    const initialActiveMembers = await page
      .locator('[data-testid="active-members-value"]')
      .textContent();
    ...
    const cachedActiveMembers = await page
      .locator('[data-testid="active-members-value"]')
      .textContent();
