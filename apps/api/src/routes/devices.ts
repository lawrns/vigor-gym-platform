...
const deviceAuthRateLimit = isTestMode
 ...
 */
router.post('/auth', deviceAuthRateLimit, async (req: Request, res: Response) => {
  try {
    ...
    if (!device) {
      ...
      return res
        .status(401)
        .json({ message: 'Invalid device credentials', code: 'INVALID_CREDENTIALS' });
    }
 ...
 */
router.get('/me', deviceAuthRequired(), async (req: DeviceAuthenticatedRequest, res: Response) => {
  try {
    const device = await prisma.device.findUnique({
