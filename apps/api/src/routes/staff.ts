...
router.post('/', authRequired(['owner', 'manager']), tenantRequired(), async (req: TenantRequest, res: Response) => {
  try {
...
router.patch('/:id', authRequired(['owner', 'manager']), tenantRequired(), async (req: TenantRequest, res: Response) => {
  try {
    ...
    if (validatedData.role && ['ADMIN', 'MANAGER'].includes(validatedData.role) && req.user!.role !== 'owner') {
...
router.delete('/:id', authRequired(['owner']), tenantRequired(), async (req: TenantRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/:id/schedule', authRequired(['owner', 'manager', 'staff']), tenantRequired(), async (req: TenantRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/:id/shifts', authRequired(['owner', 'manager']), tenantRequired(), async (req: TenantRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/:id/certifications', authRequired(['owner', 'manager']), tenantRequired(), async (req: TenantRequest, res: Response) => {
  try {
    const { id } = req.params;
