...
import companiesRoutes, {
  setPrismaInstance as setCompaniesPrismaInstance,
...
import membershipsRoutes, {
  setPrismaInstance as setMembershipsPrismaInstance,
  ...
  async (req: TenantRequest, res: Response) => {
    try {
      ...
      if (isNaN(fromDate.getTime()) || isNaN(toDate.getTime())) {
        return res
          .status(400)
          .json({ message: 'Invalid date format. Use ISO 8601 format (YYYY-MM-DDTHH:mm:ss.sssZ)' });
      }
      ...
      const [activeMembers, gyms, wellnessProviders, filteredVisits, totalVisits, monthlyRevenue] =
        await Promise.all([
          ...
          prisma.membership
            ...
            .then(memberships =>
              memberships.reduce(
                (total, membership) => total + (membership.plan.priceMxnCents || 0),
                0
              )
            ),
        ]);
...
if (process.env.NODE_ENV === 'development') {
  const server = app.listen(initialPort, () => {
    ...
    console.log(
      `⏰ Job Scheduler: ${process.env.JOBS_ENABLED !== 'false' ? 'Enabled' : 'Disabled'}`
    );
  });
  server.on('error', (err: Error & { code?: string }) => {
    if (err?.code === 'EADDRINUSE') {
      ...
      console.error(
        `   Stop the other process or update NEXT_PUBLIC_API_URL in apps/web/.env.local`
      );
