...
export function validateTenantAccess(userCompanyId: string, requestedOrgId: string): void {
  if (userCompanyId !== requestedOrgId) {
    throw new DashboardValidationError('FORBIDDEN', 'Access denied to organization data', 'orgId');
  }
}
...
export function validateDateRange(
  ...
  if (from && to) {
    ...
    if (isNaN(fromDate.getTime()) || isNaN(toDate.getTime())) {
      throw new DashboardValidationError('INVALID_RANGE', 'Invalid date format', 'dateRange');
    }
