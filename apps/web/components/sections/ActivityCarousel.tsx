  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between">
        <h2 id="activities-heading" className="font-display text-2xl text-heading">{title}</h2>
        <div className="hidden md:flex gap-2" aria-label="Controles del carrusel" role="group">
          <button aria-label="Anterior" onClick={() => scrollBy(-1)} className="rounded-md border border-[var(--outline)] px-3 py-1.5 text-sm hover:bg-[var(--surface-alt)]">Prev</button>
        ...
        {items.map((it, i) => (
          <div key={it.label} className="min-w-[75%] md:min-w-[32%] snap-start rounded-xl border border-[var(--outline)] bg-card overflow-hidden">
            <div className="relative aspect-[4/3]">
              <Image src={it.image || (i % 3 === 0 ? '/images/activity-a.png' : i % 3 === 1 ? '/images/activity-b.png' : '/images/activity-c.png')} alt={it.label} fill sizes="(max-width: 768px) 75vw, 33vw" className="object-cover" />
            </div>
