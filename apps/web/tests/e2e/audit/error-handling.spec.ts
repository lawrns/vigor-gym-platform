...
test.describe('Error Handling & Edge Cases', () => {
  test('Network failure recovery - Dashboard', async ({ page }) => {
    ...
    const errorMessages = await page.locator('[data-testid*="error"], .error, [class*="error"]').count();
    const loadingStates = await page.locator('[data-testid*="loading"], .loading, [class*="loading"]').count();
    ...
    console.log(`✅ Network failure handled: ${errorMessages} errors, ${loadingStates} loading states`);
  });
  ...
  test('Invalid form data - Onboarding', async ({ page }) => {
    ...
    const submitButtons = await page.locator('button[type="submit"], button:has-text("Continuar")').all();
    ...
    if (emailInputs.length > 0 && submitButtons.length > 0) {
      ...
      const errorMessages = await page.locator('.text-red-600, .text-red-400, [class*="error"]').count();
  ...
  test('Session expiration - Auth token invalid', async ({ page }) => {
    ...
    const hasAuthError = currentUrl.includes('/login') || 
                        await page.locator(':has-text("login"), :has-text("unauthorized"), :has-text("expired")').count() > 0;
