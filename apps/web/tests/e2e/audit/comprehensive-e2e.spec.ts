...
test.describe('Comprehensive E2E Site Testing', () => {
  ...
  test('3. Dashboard 2.0 - Widget Population and Real-time', async ({ page }) => {
    ...
    const widgets = ['Active Visits', 'Revenue', 'Activity Feed', 'Classes', 'Staff Coverage'];
    ...
    const realtimeIndicator = page
      .locator('text=Datos en tiempo real, text=real-time, text=connected')
      .first();
  ...
  test('4. Onboarding Wizard - Complete Flow', async ({ page }) => {
    ...
    // Test Brand step
    const gymNameInput = page
      .locator('input[name="gymName"], input[placeholder*="gym"], input[placeholder*="Gym"]')
      .first();
    ...
    const nextButton = page
      .locator('button:has-text("Continuar"), button:has-text("Next"), button:has-text("Continue")')
      .first();
  ...
  test('10. Performance and Console Errors - Final Check', async ({ page }) => {
    ...
    const criticalErrors = errors.filter(
      error => !error.includes('favicon') && !error.includes('404') && !error.includes('warning')
    );
