...
function analyzeResults(reportPath) {
  ...
  try {
    ...
    console.log(
      `   Success Rate: ${(100 - (report.aggregate.counters['http.request_rate'] || 0)).toFixed(2)}%`
    );
    ...
    console.log(
      `   Success Rate: ${successRate >= thresholds.successRate ? '✅' : '❌'} ${successRate.toFixed(2)}% (target: >${thresholds.successRate}%)`
    );
    console.log(
      `   Avg Response: ${avgResponseTime <= thresholds.avgResponseTime ? '✅' : '❌'} ${avgResponseTime.toFixed(2)}ms (target: <${thresholds.avgResponseTime}ms)`
    );
    console.log(
      `   95th Percentile: ${p95ResponseTime <= thresholds.p95ResponseTime ? '✅' : '❌'} ${p95ResponseTime.toFixed(2)}ms (target: <${thresholds.p95ResponseTime}ms)`
    );
    console.log(
      `   99th Percentile: ${p99ResponseTime <= thresholds.p99ResponseTime ? '✅' : '❌'} ${p99ResponseTime.toFixed(2)}ms (target: <${thresholds.p99ResponseTime}ms)`
    );
  } catch (error) {
